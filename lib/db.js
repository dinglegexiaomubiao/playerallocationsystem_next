...
const addTeam = async (team, tournamentId = null) => {
  const client = await pool.connect();
  try {
    // 获取下一个可用ID（兼容旧表无自增默认值的情况）
    const maxIdResult = await client.query('SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM public.teams');
    const nextId = parseInt(maxIdResult.rows[0].next_id, 10);
