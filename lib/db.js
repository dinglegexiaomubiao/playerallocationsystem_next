...
const initDatabase = async () => {
  ...
  try {
    ...
    // 为player_tournament_participations表添加索引
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_player_tournament_participations_player_id 
      ON public.player_tournament_participations (player_id);
    `);
    
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_player_tournament_participations_tournament_id 
      ON public.player_tournament_participations (tournament_id);
    `);
    
    await client.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS idx_player_tournament_participations_unique 
      ON public.player_tournament_participations (player_id, tournament_id);
    `);
