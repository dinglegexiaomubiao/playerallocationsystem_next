...
- 胜率最高英雄(≥10场): ${(stats.highestWinRateHeroes || []).map(h => `${h.name}(${h.matches}场/${h.winRate}%胜率)`).join(', ') || '无数据'}`;
  }
...
2. 🦸 **英雄池深度**：对比自报擅长英雄 vs 实际常用/高胜率英雄的一致性，判断英雄池是广是窄、是否存在"绝活哥"特质。近期使用的英雄是否多样化
